...
for(var i=0;i<9;i++) {
	casas[i].addEventListener('click', (event) => {
		...
		if( (event.target.value=='_') && (vencedor=='_')) {
			...
			event.target.style.color='#DC143C'; //torna o valor da casa visível
...
b_reiniciar.addEventListener('click', (event) => {
	for(var i=0;i<9;i++) {
		...
		casas[i].style.color='#FFF'; //Torna o valor _ invisível
		casas[i].style.backgroundColor='#FFF'; //Torna o fundo branco
	}
...
var sortearJogador = function() {
	if(Math.floor(Math.random() * 2)==0) {
		...
		label_jogador.style.color='#DC143C'; //deixa o texto na cor vermelha
	}else{
		jogador = "X";//define o jogador X como atual
		label_jogador.innerText="X"; //exibe na página qual é o jogador atual
		label_jogador.style.color='#DC143C'; //deixa o texto na cor azul
	}
}
...
var trocarJogador = function() {
	if(jogador=='X') {
		...
		label_jogador.style.color='#DC143C';
...
var vitoria = function() {
	if((casas[0].value==casas[1].value) && (casas[1].value==casas[2].value) && casas[0].value!='_' ) {
		casas[0].style.backgroundColor='#4D5462';
		casas[1].style.backgroundColor='#4D5462';
		casas[2].style.backgroundColor='#4D5462';

		return casas[0].value;

	}else if((casas[3].value==casas[4].value) && (casas[4].value==casas[5].value) && casas[3].value!='_' ) {
		casas[3].style.backgroundColor='#4D5462';
		casas[4].style.backgroundColor='#4D5462';
		casas[5].style.backgroundColor='#4D5462';

		return casas[3].value;

	}else if((casas[6].value==casas[7].value) && (casas[7].value==casas[8].value) && casas[6].value!='_' ) {
		casas[6].style.backgroundColor='#4D5462';
		casas[7].style.backgroundColor='#4D5462';
		casas[8].style.backgroundColor='#4D5462';
